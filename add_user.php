    ...
    if (isset($_POST["user_data_submit"])) {
        ...
        if (isset($_POST["birth_day_input"]) and !empty($_POST["birth_day_input"])) {
            ...
            if ($birth_day < 1 or $birth_day > 31) {
                $birth_day_error = "Palun vali sأ¼nni päev!";
            }
        } else {
            $birth_day_error = "Palun vali sأ¼nni päev!";
        }
        ...
        if (empty($firstname_error) and empty($surname_error) and empty($birth_month_error) and empty($birth_year_error) and empty($birth_day_error)
            and empty($birth_date_error) and empty($gender_error) and empty($email_error) and empty($password_error) and empty($confirm_password_error)) {
...
<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
    ...
    <input name="first_name_input" id="first_name_input" type="text"
           value="<?php echo $firstname; ?>"><span><?php echo $firstname_error; ?></span><br>
    <label for="surname_input">Perekonnanimi:</label><br>
    <input name="surname_input" id="surname_input" type="text"
           value="<?php echo $surname; ?>"><span><?php echo $surname_error; ?></span>
    <br>

    <input type="radio" name="gender_input" id="gender_input_1" value="2" <?php if ($gender == "2") {
        echo " checked";
    } ?>><label for="gender_input_1">Naine</label>
    <input type="radio" name="gender_input" id="gender_input_2" value="1" <?php if ($gender == "1") {
        echo " checked";
    ...
    <select name="birth_year_input">
        <option value="" selected disabed>aasta</option>
    ...
    <input name="confirm_password_input" id="confirm_password_input"
           type="password"><span><?php echo $confirm_password_error; ?></span><br>
