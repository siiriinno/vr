...
//order by added desc
//limit 5");
    $stmt = $conn->prepare("SELECT title, content, added, firstname, lastname 
FROM vr22_news 
...
LIMIT 5");
    //echo $conn->error;
    $stmt->bind_result($title_from_db, $content_from_db, $added_from_db, $first_name_from_db, $last_name_from_db);
    $stmt->execute();
    while ($stmt->fetch()) {
